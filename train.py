...
from models.scl import build_model
import utils
...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    for i,(image, caption, img_pid, caps) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_glb, loss_loc, loss_ara= model(image, caption, alpha, img_pid, caps=caps)
        loss_glb = loss_glb * 1.
        loss_loc = loss_loc * 1.
...
def evaluation(model, data_loader, device, config, itm=False):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_input = model.tokenizer(text, padding='max_length', truncation=True, max_length=56, return_tensors="pt").to(device)
